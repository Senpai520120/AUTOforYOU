...
| Находка | Где |
|---|---|
| B-01 `DEBUG=true` в Docker, CORS открыт всем | PR #17 |
| B-02 Эскалация привилегий через `role` | PR #17 |
| B-03 Скрытые объявления читаются анонимом | PR #25 |
| B-04 LiqPay: валидация ввода (сумма от клиента — в #24) | PR #25 |
| B-05 Инвалидация кэша тарифов между процессами | PR #33 |
| B-06 README оставлял калькулятор нерабочим | PR #34 |
| B-07 Тесты стучались в боевой Telegram | PR #33 |
| B-08 `seed_rates` падал при недоступном Redis | PR #33 |
| B-09 N+1 на каталоге: 23 запроса стали 3 | PR #32 |
| B-10 Celery в eager-режиме в Docker | PR #17 |
| B-11 CI не запускал тесты, `\|\| true` | PR #21 |
| B-12 Telegram-webhook без проверки секрета | PR #33 |
| B-13 Публичные `shipments` с VIN | PR #25 |
| B-14 `VinReportView` без auth и throttle | PR #33 |
| Фальшиво-зелёные тесты, `label` не связан с полем | PR #15 |
| Каталоги не рендерились на сервере | PR #28 |
| Пустая главная страница | PR #19 |
| Юридический блок: согласие, возврат, индексация | PR #12 |
| `openapi.yaml` описывал 13 путей из 60 | PR #34 |
| README описывал 7 приложений из 16 | PR #34 |
| `Listing.status` принимал значения вне `choices` | PR #30 |
| Согласие дилера не фиксировалось на сервере | PR #31 |
...
| Сумма в LiqPay-чекауте приходит от клиента | Нет серверного прайса — требует решения по продукту, #24 |
| Реквизиты компании | Нужны данные владельца, #10 |
| Авторские права на фото Copart/IAAI | Вопрос к юристу, #11 |

Из 26 отслеженных находок закрыто 22. Три из четырёх оставшихся упираются в решения владельца, а не в код.
